...
static int compareFieldIds(const DexFieldId* pFieldId_1, const DexFieldId* pFieldId_2) {
...
static int compareProtoIds(const DexFile* pDex_1, const DexProtoId* pProtoId_1,
                           const DexFile* pDex_2, const DexProtoId* pProtoId_2) {
...
static void mapProtoIds(const DexFile* pBaseDex, const DexFile* pVariantDex) {
  ...
  while (i < baseProtoIdsSize && j < variantProtoIdsSize) {
    ...
    int cmp = compareProtoIds(pBaseDex, pBaseProtoId, pVariantDex, pVariantProtoId);
    if (cmp < 0) {
      i++;
      isNeutralized = false;
    } else if ( cmp > 0 ) {
...
static void neutralizeFieldId(DexFieldId* pFieldId) {
  ...
  // neutralize nameIdx
  uint32_t newNameIdx;
  if (hashmap_get(stringIdsMap, pFieldId->nameIdx, &newNameIdx) == MAP_OK) {
    pFieldId->nameIdx = newNameIdx;
  }

  // neutralize classIdx
  uint32_t newClassIdx;
  if (hashmap_get(typeIdsMap, pFieldId->classIdx, &newClassIdx) == MAP_OK) {
...
static void neutralizeMethodIds(DexFile* pBaseDex, const DexFile* pVariantDex) {
...
void neutralizeClassInterfaces(const DexFile* pBseDex, const DexClassDef* pClassDef) {
  ...
  if (pTypeList) {
    for (uint32_t j = 0; j < pTypeList->size; ++j) {
      ...
      if (hashmap_get(typeIdsMap, typeId, &newInterfaceIdx) == MAP_OK) {
...
void neutralizeClassDefs(DexFile* pBaseDex) {
  ...
  for (uint32_t i = 0; i < size; ++i) {
    ...
    // classIdx
    if (hashmap_get(typeIdsMap, pClassDef->classIdx, &newIdx) == MAP_OK) {
      pClassDef->classIdx = newIdx;
    }

    //super class Idx
    if (hashmap_get(typeIdsMap, pClassDef->superclassIdx, &newIdx) == MAP_OK) {
    ...
    // annotations_off
    DexAnnotationsDirectoryItem* pDirectory = dexGetAnnotationsDirectoryItem(pBaseDex, pClassDef);
    if (pDirectory != nullptr) {
      ...
      if (pFieldItems != NULL) {
        for (uint32_t j = 0; j < pDirectory->fieldsSize; ++j) {
          ...
          neutralizeAnnotationsSet(pBaseDex, pFieldItems->annotationsOff);
        }
      }

      // annotations_off::method_annotations
      DexMethodAnnotationsItem* pMethodItems = dexGetMethodAnnotations(pBaseDex, pDirectory);
      if (pMethodItems != NULL) {
        for (uint32_t j = 0; j < pDirectory->methodsSize; ++j) {
          if (hashmap_get(methodIdsMap, pMethodItems->methodIdx, &newIdx) == MAP_OK) {
            pMethodItems->methodIdx = newIdx;
          }
          neutralizeAnnotationsSet(pBaseDex, pMethodItems->annotationsOff);
        }
      }

      // annotations_off::parameters_annotations
      DexParameterAnnotationsItem* pParamItems = dexGetParameterAnnotations(pBaseDex, pDirectory);
      if (pParamItems != NULL) {
        for (uint32_t j = 0; j < pDirectory->parametersSize; ++j) {
          ...
          for (uint32_t k = 0; k < pRefList->size; ++k) {
            neutralizeAnnotationsSet(pBaseDex, pRefList->list[k].annotationsOff);
          }
        }
      }
    }
    ...
    if (pData != NULL) {
      ...
      if (pInstanceFields != NULL) {
        for (uint32_t j = 0; j < pClassData->header.instanceFieldsSize; ++j) {
          if (hashmap_get(fieldIdsMap, pInstanceFields->fieldIdx, &newIdx) == MAP_OK) {
      ...
      if (pStaticFields != NULL) {
        for (uint32_t j = 0; j < pClassData->header.staticFieldsSize; ++j) {
          if (hashmap_get(fieldIdsMap, pStaticFields->fieldIdx, &newIdx) == MAP_OK) {
      ...
      if (pDirectMethods != NULL) {
        for (uint32_t j = 0; j < pClassData->header.directMethodsSize; ++j) {
          if (hashmap_get(methodIdsMap, pDirectMethods->methodIdx, &newIdx) == MAP_OK) {
      ...
      if (pVirturalMethods != NULL) {
        for (uint32_t j = 0; j < pClassData->header.directMethodsSize; ++j) {
          if (hashmap_get(methodIdsMap, pVirturalMethods->methodIdx, &newIdx) == MAP_OK) {
