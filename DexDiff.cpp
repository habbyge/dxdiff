...
static int loadDexFile(const char* fileName, DexInput* input) {
  ...
  if (dexOpenAndMap(fileName, NULL, &input->map, false) != 0) {
    fprintf(stderr, "Failed to map the dex file '%s' in memory", fileName);
    return 1;
  }
  input->isMapped = true;

  //parsing dex file
  input->dexFile = dexFileParse((u1*) input->map.addr, input->map.length, kDexParseVerifyChecksum);
  if (input->dexFile == nullptr) {
...
int main(int argc, char* const argv[]) {
  ...
  if (options.neutralize) {
    sysChangeMapAccess(baseDexInput.map.addr, baseDexInput.map.length, true, &baseDexInput.map);
    neutralizeBase(baseDexInput.dexFile, newDexInput.dexFile);
    sysChangeMapAccess(baseDexInput.map.addr, baseDexInput.map.length, false, &baseDexInput.map);
  }
